...
print_result(f"Created report ID: {report_id}")
...
while report_status.value < ReportStatusV2.Failed.value:
    ...
    print_result(f"Status: {report_status} - Progress {progress}")
...
if report_status == ReportStatusV2.Failed:
    print_result("Report creation failed")
    exit(report_status)

if report_status == ReportStatusV2.Deleted:
    print_result("Report was already deleted")
    exit(report_status)

if report_status == ReportStatusV2.Ready:
    print_result("Report is ready for download")
...
print_result(F"Report downloaded to {report_path}")
